...
class OpenAIGPT41Provider extends BaseProvider {
  defaultBaseUrl: string;

  constructor(config: OpenAIAdapterConfig = {}) {
    super({
      id: config.id || 'openai_gpt4_1',
      name: config.name || 'OpenAI GPT-4.1',
      models: config.models || ['gpt-4.1', 'gpt-4o', 'gpt-4.5-preview'],
  ...
  async *handleChat(messages: ChatMessage[], config: OpenAIAdapterConfig, options: OpenAIAdapterOptions = {}): AsyncIterable<StreamEvent> {
    ...
    if (targetModel === 'default') {
      targetModel = config.DEFAULT_MODEL || config.defaultModel || this.defaultModel || 'gpt-4.1';
    }
...
export = new OpenAIGPT41Provider();
