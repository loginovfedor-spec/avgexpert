...
const { DEFAULT_SYSTEM_PROMPT } = require('../../core/config');
const { getGuestAllowedCategories } = require('./users.routes');
const { USERNAME_MESSAGE } = require('../../core/utils');
...
router.post('/register', asyncHandler(async (req, res) => {
  ...
  if (existingEmailUser) {
    AuditService.log(existingEmailUser.username, 'REGISTER_FAILED', { reason: 'email_exists', attempted_username: username }, ip);
  ...
  if (selectedCategory) {
    ...
    selectedCategory = guestAllowedCategories[0] || null;
  }
  ...
  res.status(201).json({ detail: 'Пользователь успешно зарегистрирован' });
}));
