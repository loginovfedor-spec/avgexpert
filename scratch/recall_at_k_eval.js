 ...
 *
 * Запуск:
 *   node scratch/recall_at_k_eval.js
 *   node scratch/recall_at_k_eval.js --json=scratch/recall_at_k_report.json
 *   node scratch/recall_at_k_eval.js --validate-only
 *   node scratch/recall_at_k_eval.js --only=yandex-rag,qwen-8b
 */
...
const { validateDataset } = require('../tests/evals/rag_recall.eval');
...
const corpus = require('../tests/evals/rag_recall_corpus.json');
const queries = require('../tests/evals/rag_recall_queries.json');
...
async function yandexEmbed(text, suffix) {
...
async function qwenEmbed(texts, dims) {
...
async function embedCorpus(provider) {
  const embeddings = {};
  ...
  if (provider.role === 'qwen') {
    ...
    for (let i = 0; i < corpus.length; i += BATCH) {
      ...
      const embs = await qwenEmbed(batch.map(c => c.text), provider.dims);
      batch.forEach((c, j) => { embeddings[c.id] = embs[j]; });
...
async function evalProvider(provider) {
  ...
  const asyncReport = { queries: [], summary: { recall_at_3: 0, recall_at_7: 0, mrr: 0, count: queries.length } };
  let r3 = 0;
  let r7 = 0;
  let mrrSum = 0;
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    const { rankChunks, recallAtK, mrr } = require('../tests/evals/recall_metrics');
...
async function main() {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      results[provider.id] = { error: err.message };
    }
  }

  const baseline = results['yandex-rag'];
  const candidate = results['qwen-8b'];
  let gate = null;
  if (baseline?.summary && candidate?.summary) {
    ...
    log(`\nGate: ${gate.passed ? 'PASS' : 'FAIL'} (candidate r@3 ${gate.candidate_recall_at_3.toFixed(3)} vs baseline ${gate.baseline_recall_at_3.toFixed(3)})`);
  }
