 ...
 *
 * Запуск:
 *   tsx scratch/compare_embeddings.ts
 *   tsx scratch/compare_embeddings.ts --only=openai-small,yandex-rag
 *   tsx scratch/compare_embeddings.ts --json=scratch/embed_results.json --quiet
 */
...
const MODELS: ModelConfig[] = [
  ...
  {
    id: 'yandex-search-doc',
    provider: 'yandex',
    name: 'Yandex text-search-doc',
    modelSuffix: YANDEX_DOC_SUFFIX,
    role: 'doc',
  },
  {
    id: 'yandex-search-query',
    provider: 'yandex',
    name: 'Yandex text-search-query',
    modelSuffix: YANDEX_QUERY_SUFFIX,
    role: 'query',
  },
  {
    id: 'yandex-v2-doc',
    provider: 'yandex',
    name: 'Yandex text-embeddings-v2-doc',
    modelSuffix: 'text-embeddings-v2-doc/latest',
    role: 'doc',
  },
...
type BenchmarkText = { text: string; role: 'query' | 'doc' | 'any' };

function collectBenchmarkTexts(): BenchmarkText[] {
  const items: BenchmarkText[] = [];
...
function dedupeTexts(items: BenchmarkText[]): {
  unique: BenchmarkText[];
  indexByKey: Map<string, number>;
} {
  const unique: BenchmarkText[] = [];
...
async function getYandexEmbedding(text: string, modelSuffix: string, retries = 3): Promise<number[]> {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    const data = (await resp.json()) as { embedding?: number[]; message?: string };
  ...
  throw new Error(`Yandex [${modelSuffix}]: exhausted retries`);
}

async function embedYandexSequential(uniqueItems: BenchmarkText[], modelSuffix: string): Promise<number[][]> {
...
async function embedYandexRagBatch(uniqueItems: BenchmarkText[]): Promise<number[][]> {
  const results: number[][] = [];
  for (let i = 0; i < uniqueItems.length; i++) {
    if (i > 0) await sleep(120);
...
async function getBatchEmbeddings(uniqueItems: BenchmarkText[], model: ModelConfig): Promise<number[][]> {
...
function getEmb(embMap: Map<string, number[]>, text: string, role: 'query' | 'doc' | 'any' = 'any'): number[] {
  return embMap.get(`${role}\0${text}`) || embMap.get(`any\0${text}`)!;
}

function canRunModel(model: ModelConfig): string | null {
...
function evaluateModel(embMap: Map<string, number[]>): EvalMetrics {
  ...
  return {
    closeAvg,
    closeMin: Math.min(...closeScores),
    closeMax: Math.max(...closeScores),
    closeStd: stddev(closeScores),
    farAvg,
    farMin: Math.min(...farScores),
    farMax: Math.max(...farScores),
    farStd: stddev(farScores),
    margin,
    rankCorrect,
    rankTotal,
    rankGap,
...
async function runBenchmark(): Promise<{
  ...
  for (const model of models) {
    ...
    if (skipReason) {
      ...
      results[model.id] = { error: skipReason } as ModelResult;
      continue;
    }
    ...
    try {
      ...
      results[model.id] = { error: message } as ModelResult;
      continue;
    }
    ...
    log(
      `  Margin=${metrics.margin.toFixed(4)}  Rank=${metrics.rankCorrect}/${metrics.rankTotal}  Gap=${metrics.rankGap.toFixed(4)}  Score=${metrics.score.toFixed(4)}`
    );
    ...
    if (!QUIET) {
      ...
      for (const rd of metrics.rankDetails) {
        ...
        log(
          `  [${mark}] g=${rd.simGood.toFixed(4)} m=${rd.simMediocre.toFixed(4)} b=${rd.simBad.toFixed(4)}  «${rd.query.slice(0, 50)}»`
        );
      }
    }

    const { rankDetails: _rankDetails, ...metricsWithoutDetails } = metrics;
    results[model.id] = {
      name: model.name,
  ...
  const valid: Array<{ model: ModelConfig; r: ModelResult }> = [];
  for (const model of models) {
  ...
  if (valid.length > 0) {
    ...
    if (prodDefault && !prodDefault.error) {
      console.log(
        `  Production RAG (yandex-rag): margin=${prodDefault.margin!.toFixed(4)}, rank=${prodDefault.rankCorrect}/${prodDefault.rankTotal}, score=${prodDefault.score!.toFixed(4)}`
      );
    }
