...
import { $, t } from './index';
import { newChat, addMessageToUI } from './chat';
import { updateContextBadge, showToast, setWelcomeVisible } from './ui';
import { stopAllAttachmentPolling } from './session-attachments';
...
export const SessionManager = {
    ...
    try {
      ...
      if (r.ok) {
        ...
        localStorage.setItem(
          `avgexpert_${state.currentUser?.username}_sessions_cache`,
      ...
      const cached = localStorage.getItem(`avgexpert_${state.currentUser?.username}_sessions_cache`);
      if (cached) this.renderList(JSON.parse(cached) as SessionSummary[]);
    }
  },
  ...
  async loadSession(id: string) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        state.chatHistory.forEach(m => addMessageToUI(m.role, m.content, m.docs || [], m.categoryName || ''));

        const chatSessionCat = $<HTMLSelectElement>('chat-session-category');
        if (chatSessionCat) {
          chatSessionCat.value = data.category || state.currentUser?.category || '';
        }
  ...
  async deleteSession(id: string, ev?: Event) {
  ...
  async _fetchMessages(id: string): Promise<SessionData | null> {
    const r = await fetch('/api/sessions/' + id, {
      headers: { 'Authorization': 'Bearer ' + state.authToken || '' }
    });
    if (!r.ok) return null;
  ...
  _buildCsv(sessionData: SessionData) {
    const rows: string[][] = [['Дата-Время', 'Вопрос', 'Ответ']];
  ...
  _downloadCsv(filename: string, csvContent: string) {
    const bom = '\uFEFF';
  ...
  async exportSession(id: string, ev?: Event) {
    if (ev) ev.stopPropagation();
    try {
  ...
  async deleteGroup(_groupName: string, items: SessionSummary[]) {
  ...
  async exportGroup(groupName: string, items: SessionSummary[]) {
    try {
      const allRows: string[][] = [['Дата-Время', 'Чат', 'Вопрос', 'Ответ']];
  ...
  renderList(sessions: SessionSummary[]) {
...
window.addEventListener('storage', (ev: StorageEvent) => {
  if (ev.key === 'avgexpert_token' && !ev.newValue) {
