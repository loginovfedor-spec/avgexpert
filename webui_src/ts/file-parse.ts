...
function loadModuleOnce<T>(key: string, importer: () => Promise<T>): Promise<T> {
  if (pendingModuleLoads.has(key)) return pendingModuleLoads.get(key) as Promise<T>;
...
export function filterSupportedDocumentFiles(fileList: FileList | File[]): File[] {
