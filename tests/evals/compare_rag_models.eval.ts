 ...
 *
 * Запуск:
 *   tsx tests/evals/compare_rag_models.eval.ts
 *   tsx tests/evals/compare_rag_models.eval.ts --only=qwen-max,alice-flash
 *   tsx tests/evals/compare_rag_models.eval.ts --json=scratch/rag_model_results.json --quiet
 *   npm run bench:rag-tier
 */
import fs from 'fs';
...
dotenv.config({ path: path.join(ROOT, 'src/modules/providers/config/openai_gpt4_1.env') });
dotenv.config({ path: path.join(ROOT, 'src/modules/providers/config/openai_gpt5_5.env') });
dotenv.config({ path: path.join(ROOT, 'src/modules/providers/config/yandex_file_search.env') });
dotenv.config({ path: path.join(ROOT, 'src/modules/providers/config/qwen.env') });
dotenv.config({ path: path.join(ROOT, 'src/modules/providers/config/grok.env') });
...
type ModelDef = {
  id: string;
  label: string;
  provider: 'openai' | 'yandex' | 'qwen' | 'grok';
  model: string;
  fallbacks?: string[];
  costIn?: number;
  costOut?: number;
...
const MODELS: ModelDef[] = [
  ...
  { id: 'qwen-flash', label: 'Qwen3.6-Flash', provider: 'qwen', model: 'qwen-flash', fallbacks: ['qwen-turbo'] },
  {
    ...
    fallbacks: ['grok-4-fast-non-reasoning'],
    costIn: 0.2,
    costOut: 0.5,
  },
  {
    id: 'grok-41-reasoning',
    label: 'Grok 4.1 Reasoning',
    provider: 'grok',
    model: 'grok-4-1-fast-reasoning',
    fallbacks: ['grok-4-fast-reasoning'],
    costIn: 0.2,
    costOut: 0.5,
  },
  { id: 'grok-43', label: 'Grok 4.3', provider: 'grok', model: 'grok-4.3', fallbacks: ['grok-4'], costIn: 3.0, costOut: 15.0 },
  {
    ...
    provider: 'grok',
    model: 'grok-3-mini',
    fallbacks: ['grok-3-mini-beta'],
    costIn: 0.3,
    costOut: 0.5,
  },
];

const TIER_BASELINE: Record<string, string[]> = {
...
const COST_PER_1M: Record<string, { in: number; out: number }> = {
  ...
  'grok-41-fast': { in: 0.2, out: 0.5 },
  'grok-41-reasoning': { in: 0.2, out: 0.5 },
...
type RagTest = {
  ...
  refuseMarkers?: string[];
};

const RAG_TESTS: RagTest[] = [
  {
    id: 'R1',
    type: 'answer',
    context:
      'Срок возврата товара — 14 календарных дней с момента получения. Деньги возвращаются на карту в течение 10 рабочих дней после одобрения заявки.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Доставка по Москве — 299 рублей, бесплатно при заказе от 5000 рублей. Доставка в регионы — от 499 рублей, срок 3–7 рабочих дней.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Для восстановления пароля перейдите в «Настройки → Безопасность → Сменить пароль» или нажмите «Забыли пароль» на странице входа. Ссылка действует 24 часа.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Подписка Premium: 499 руб/мес, отмена в любой момент в личном кабинете. При отмене доступ сохраняется до конца оплаченного периода.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Гарантия на электронику — 12 месяцев с даты покупки. На аксессуары — 6 месяцев. Гарантийный ремонт по адресу: ул. Ленина, 15.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Способы оплаты: банковская карта (Visa, MasterCard, МИР), СБП, наличные курьеру. Рассрочка 0% на 6 месяцев при заказе от 10000 руб.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Чтобы изменить адрес доставки, обратитесь в поддержку до 18:00 дня отправки заказа. После отгрузки изменение невозможно.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Самовывоз из пункта выдачи бесплатный. Адрес ПВЗ: г. Москва, ул. Тверская, 10. Режим работы: пн-вс 10:00–22:00.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Для оформления рассрочки нужен паспорт и справка о доходах. Решение банка — до 15 минут. Первый платёж через 30 дней.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Максимальное рабочее давление RX-900 — 150 МПа. Превышение запрещено согласно разделу 4.2 руководства по безопасности.',
  ...
  },
  {
    ...
    type: 'answer',
    context:
      'Для получения паспорта: заявление, свидетельство о рождении, 2 фото 35×45 мм, квитанция об оплате госпошлины 3000 рублей.',
...
type BenchmarkResult = {
  ...
  openaiPromptCache: Record<string, unknown> | null;
  score: number;
  tests: TestResult[];
  elapsedSec?: number;
  error?: string;
...
const llmCache = new Map<string, CachePayload>();

function cacheKey(modelId: string, system: string, userContent: string) {
...
function setCached(key: string, value: CachePayload) {
...
function padR(s: unknown, n: number) {
  const str = String(s);
  return str.length >= n ? str + ' ' : str + ' '.repeat(n - str.length);
}

function normalizeUsage(usage: Record<string, unknown> | null | undefined): Usage {
  ...
  const inputDetails = usage.input_tokens_details as { cached_tokens?: number } | undefined;
  return {
    ...
    cached_tokens: promptDetails?.cached_tokens || inputDetails?.cached_tokens || (usage.cached_tokens as number) || 0,
  };
}

function scoreAnswer(test: RagTest, answer: string) {
  const text = answer.toLowerCase();
  if (test.type === 'refuse') {
    const refused = (test.refuseMarkers || []).some((m) => text.includes(m));
    const hallucinated =
      /\b(иван|петр|sergey|ceo|директор [а-я]+ [а-я]+)\b/i.test(answer) &&
...
function buildUserMessage(context: string, query: string) {
...
const openaiClient = OPENAI_API_KEY ? new OpenAI({ apiKey: OPENAI_API_KEY, baseURL: OPENAI_BASE_URL }) : null;

const yandexClient =
  YANDEX_API_KEY && YANDEX_FOLDER
    ? new OpenAI({
...
const grokClient = GROK_API_KEY ? new OpenAI({ apiKey: GROK_API_KEY, baseURL: GROK_BASE_URL }) : null;

function extractResponseText(response: { output?: Array<{ type?: string; content?: Array<{ type?: string; text?: string }> }> }) {
  let text = '';
  if (response.output) {
...
async function callOpenAI(
  modelDef: ModelDef,
  userContent: string,
  opts: { modelOverride?: string; promptCacheKey?: string; promptCacheRetention?: string } = {}
  ...
  const response = await openaiClient!.responses.create(params as never);
  return { text: extractResponseText(response), usage: normalizeUsage(response.usage as unknown as Record<string, unknown>) };
}

async function callYandex(modelDef: ModelDef, userContent: string): Promise<{ text: string; usage: Usage }> {
...
function estimateCostUsd(modelId: string, usage: Usage | undefined) {
...
async function callModel(modelDef: ModelDef, userContent: string, opts: Record<string, unknown> = {}): Promise<CallResult> {
  ...
  for (const modelName of modelsToTry) {
    try {
      if (modelDef.provider === 'openai') {
        return {
          ...(await callOpenAI(modelDef, userContent, { ...opts, modelOverride: modelName })),
          modelUsed: modelName,
        };
      }
      ...
      if (modelDef.provider === 'qwen') {
        return {
          ...(await callQwen(modelDef, userContent, { ...opts, modelOverride: modelName })),
          modelUsed: modelName,
        };
      }
      if (modelDef.provider === 'grok') {
        return {
          ...(await callGrok(modelDef, userContent, { ...opts, modelOverride: modelName })),
          modelUsed: modelName,
        };
      }
    } catch (err) {
...
function canRun(model: ModelDef): string | null {
...
async function runWithCache(modelDef: ModelDef, userContent: string): Promise<CachePayload> {
  ...
  const payload: CachePayload = { ...result, cacheHit: false, latencyMs };

  if (!SKIP_CACHE) setCached(key, payload);
  return payload;
}

async function benchmarkModel(modelDef: ModelDef): Promise<BenchmarkResult | { error: string }> {
  ...
  for (const test of RAG_TESTS) {
    ...
    log(
      `  [${evalCold.pass ? '✓' : '✗'}] ${test.id} cold=${cold.latencyMs}ms warm=${warm.latencyMs}ms cache=${warm.cacheHit ? 'HIT' : 'MISS'}`
    );

    if (modelDef.provider === 'yandex') await sleep(100);
  }

  let openaiPromptCache: Record<string, unknown> | null = null;
  ...
  const valueScore = totalCostUsd > 0 ? parseFloat((accuracy / totalCostUsd).toFixed(1)) : null;
...
export async function runCompareRagModels() {
  let models: ModelDef[];
  if (INJECT_ONLY) {
  ...
  if (models.length === 0) {
    throw new Error(`Нет моделей. IDs: ${MODELS.map((m) => m.id).join(', ')}`);
  }
  ...
  const tierResults: Record<string, Record<string, BenchmarkResult | { error: string }>> | null = INJECT_ONLY ? {} : null;
  ...
  for (const model of models) {
    ...
    try {
      ...
      if ('error' in bench) {
        results[model.id] = bench;
        continue;
      }
      bench.elapsedSec = parseFloat(((Date.now() - t0) / 1000).toFixed(1));
      results[model.id] = bench;
      if (tierResults) {
        for (const [tier, ids] of Object.entries(TIER_BASELINE)) {
          if (ids.includes(model.id)) {
            if (!tierResults[tier]) tierResults[tier] = {};
            tierResults[tier][model.id] = bench;
          }
        }
      }
      log(`\n  Accuracy: ${(bench.accuracy * 100).toFixed(0)}%  Score: ${bench.score}`);
      log(
        `  Latency: cold=${bench.avgColdMs}ms warm=${bench.avgWarmMs < 1 ? '<1ms (cache HIT)' : `${bench.avgWarmMs}ms`}`
      );
    } catch (err) {
  ...
  const valid: Array<{ model: ModelDef; r: BenchmarkResult }> = [];
  for (const model of models) {
  ...
  if (valid.length > 0) {
    ...
    const byValue = [...valid].filter((v) => v.r.valueScore != null).sort((a, b) => b.r.valueScore! - a.r.valueScore!);
    ...
    if (byValue.length > 0) {
      console.log(
        `  Лучшая цена/качество: ${byValue[0].model.label} (value=${byValue[0].r.valueScore}, cost=$${byValue[0].r.estCostUsd})`
      );
    }
    console.log(`  Кеш (warm):     все модели — 100% HIT при повторе (локальный SHA256-кеш)`);

    const openaiWithCache = valid.filter((v) => v.r.openaiPromptCache && !('error' in (v.r.openaiPromptCache || {})));
    if (openaiWithCache.length > 0) {
      console.log('\n  OpenAI prompt_cache_key (2-й запрос с тем же prefix):');
      for (const { model, r } of openaiWithCache) {
        const pc = r.openaiPromptCache as { run2Cached?: number; run2PromptTokens?: number };
