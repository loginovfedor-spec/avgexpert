...
import { evaluateRetrieval } from './recall_metrics';
...
export const BASELINE_RECALL_AT_3_LIVE = 0.85;
...
export async function runConsultantRecallEval() {
  validateDataset();
  ...
  for (const row of queryResults) {
    ...
    recall3Sum += row.relevant_chunk_ids.length ? hits.length / row.relevant_chunk_ids.length : 0;
  }
  const recallAt3 = recall3Sum / (queries.length || 1);

  const directReport = evaluateRetrieval(queries as RecallQuery[], embeddings, hashEmbed);
